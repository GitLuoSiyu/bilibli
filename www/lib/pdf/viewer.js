...
function parseQueryStringInvoice(query) {
    var parts = query.split('&');
    var params = {};
    for (var i = 0, ii = parts.length; i < ii; ++i) {
        var param = parts[i].split('=');
        var key = param[0].toLowerCase();
        //var value = param.length > 1 ? param[1] + "=" + param[2] + "=" + param[3] : null;
        var value = param.length > 1 ? param[1] + "=" + param[2] : null;
        params[decodeURIComponent(key)] = decodeURIComponent(value);
    }
    return params;
}
...
function webViewerInitialized() {
  ...
  var params = parseQueryStringInvoice(queryString);
